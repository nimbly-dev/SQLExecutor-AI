...
async def update_settings(tenant_id: str, settings_request: AddSettingToTenantRequest):
    return await TenantSettingsService.update_settings_to_tenant(tenant_id=tenant_id,update_settings_request=settings_request)

@router.delete("/settings/{tenant_id}/{setting_key}")
async def delete_settings(tenant_id: str, setting_key: str):
    return await TenantSettingsService.delete_setting(tenant_id=tenant_id,setting_key=setting_key)

@router.get("/settings/{tenant_id}")
async def get_settings(tenant_id: str):
    return await TenantSettingsService.get_settings(tenant_id=tenant_id)

@router.get("/settings/{tenant_id}/{setting_key}")
async def delete_settings(tenant_id: str, setting_key: str):
    return await TenantSettingsService.get_setting_detail(tenant_id=tenant_id, setting_key=setting_key)